...
def test_install_and_toggle_plugin(tmp_path, monkeypatch):
    ...
    plugin_src.write_text(
        "TOOL_METADATA={'name':'demo','description':'d','dependencies':['pkg'],'needs_config':True}\n\n"
        "def run_tool(args):\n    return 'ok'"
    )
    ...
    demo = next(t for t in tools_list if t["name"] == "demo")
    assert demo["dependencies"] == ["pkg"]
    assert demo["needs_config"] is True
