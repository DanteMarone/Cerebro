...
from PyQt5.QtWidgets import (
    ...
    QShortcut, QSystemTrayIcon, QStyle
)

from theme_utils import load_style_sheet
from log_utils import setup_logging, get_log_file_path
...
class AIChatApp(QMainWindow):
    def __init__(self):
        ...
        # self.agents_data delegated to Orchestrator
        ...
        self.orchestrator = Orchestrator(self.tools, self.tasks, self.metrics, self.debug_enabled, self.api_url, self.screenshot_manager)
        ...
        self.orchestrator.typing_stopped.connect(self.chat_tab.hide_typing_indicator)
        self.orchestrator.message_received.connect(self.on_orchestrator_message_received)
        self.orchestrator.chunk_received.connect(self.on_orchestrator_chunk_received)
        self.orchestrator.error_occurred.connect(self.on_orchestrator_error)
        self.orchestrator.all_agents_finished.connect(self.on_orchestrator_finished)
        self.orchestrator.refresh_agents_requested.connect(self.on_agents_refreshed)
        self.orchestrator.task_scheduled.connect(lambda msg: self.show_notification(msg, "info"))
        ...
        self.load_settings()
        # populate_agents is handled by Orchestrator
    ...
    def agents_data(self):
        return self.orchestrator.agents_data

    @agents_data.setter
    def agents_data(self, value):
        self.orchestrator.agents_data = value
    ...
    def send_message(self, user_text):
        ...
        success, msg = self.orchestrator.handle_user_message(user_text)
        if not success:
    ...
    def on_orchestrator_message_received(self, html_content, agent_name, agent_color):
        self.chat_tab.append_message_html(html_content)

    def on_orchestrator_chunk_received(self, chunk, agent_name):
        # Implement streaming display here if desired
        pass

    def on_orchestrator_error(self, message):
        timestamp = datetime.now().strftime("%H:%M:%S")
        log_link = QUrl.fromLocalFile(get_log_file_path()).toString()
        self.chat_tab.append_message_html(
            f"[{timestamp}] <span style='color:red;'>{message} "
            f"<a href='{log_link}'>View Logs</a></span>"
        )
        if self.chat_tab.last_user_message_id:
            self.chat_tab.update_message_status(self.chat_tab.last_user_message_id, "failed")
        self.show_notification(f"Error: {message}", "error")
    ...
    def add_agent(self):
        ...
        if ok and agent_name.strip():
            ...
            success, msg = self.orchestrator.add_agent(agent_name)
            if success:
                if self.debug_enabled:
    ...
    def delete_agent(self, agent_name=None):
        ...
        success, msg = self.orchestrator.delete_agent(agent_name)
        if success:
            if self.debug_enabled:
    ...
    def check_for_due_tasks(self):
        ...
        for t in self.tasks:
            ...
            if now >= due_dt:
                ...
                # Delegate to orchestrator
                self.orchestrator.schedule_user_message(
                    agent_name,
                    prompt,
                    t["id"],
    ...
    def load_settings(self):
        ...
        # Update orchestrator
        self.orchestrator.set_summarization_threshold(self.summarization_threshold)
